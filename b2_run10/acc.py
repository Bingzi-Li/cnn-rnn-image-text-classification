...
name_ = ['Word_RNN']
improve_type_ = ['Vanilla', 'LSTM', '2Layers']
use_dropout = False
...
def plot_b6():
    for name in name_:
        for improve_type in improve_type_:
            if use_dropout:
                hist_path = f'./b2_run10/histories/partb1_{name}_{improve_type}_dropout'
            else:
                hist_path = f'./b2_run10/histories/partb1_{name}_{improve_type}_no_dropout'
...
def print_b1_acc():
    ...
    for name in name_:
        if use_dropout:
            hist_path = f'./b2_run10/histories/partb1_{name}_dropout'
        else:
            hist_path = f'./b2_run10/histories/partb1_{name}_no_dropout'
        with open(hist_path, 'rb') as file:
            history = pickle.load(file)
            accs[hist_path] = max(
                history['test_accuracy'])
