...
name_ = ['Char_CNN', 'Word_CNN', 'Char_RNN', 'Word_RNN']
...
def plot_b6():
    for name in name_:
        for improve_type in improve_type_:
            if use_dropout:
                hist_path = f'./b2_run6/histories/partb1_{name}_{improve_type}_dropout'
            else:
                hist_path = f'./b2_run6/histories/partb1_{name}_{improve_type}_no_dropout'
    ...
    plt.xlabel('epoch')
    plt.legend()
    # plt.savefig(f'./a1_run6/results/{num_ch_c1}_{num_ch_c2}_{optimizer_}_no_dropout_acc.pdf')
    plt.show()
...
def print_b1_acc():
    ...
    for name in name_:
        if use_dropout:
            hist_path = f'./b2_run6/histories/partb1_{name}_dropout'
        else:
            hist_path = f'./b2_run6/histories/partb1_{name}_no_dropout'
        with open(hist_path, 'rb') as file:
            history = pickle.load(file)
            accs[hist_path] = max(
                history['test_accuracy'])
